...
class FastnersHandTools(Spider):
    name = "fastners_homedepot_crawl"
    ...
    #     yield Request(url=self.api, method='POST', body=self.payload, headers={'x-experience-name': 'fusion-gm-pip-desktop','content-type':'application/json'}, callback=self.parse)

    # def parse(self, response):
    #     json_data = response.json()
    #     skus = json_data['data']['metadata']['childItemsLookup']
    #     for sku in skus:
    #         yield Request(url=sku['canonicalUrl'], method='GET')
